...
def apply_map(values: pd.Series, mapping: pd.Series, prior: float) -> np.ndarray:
    return values.astype("float64").fillna(MISS).map(mapping).fillna(prior).to_numpy(np.float32)


def fold_te(train: pd.DataFrame, y: np.ndarray, valid: pd.DataFrame, test: pd.DataFrame,
            inner_folds: int, smoothing: float, seed: int):
    prior = float(np.mean(y))
    ...
    for col in NUM:
        ...
        for ti, vi in inner.split(train, y):
            ...
            oof[vi] = apply_map(train.iloc[vi][col], mp, prior)
...
def best_rotating_blend(y, folds, pred_lgb, pred_xgb):
    ...
    for held in sorted(np.unique(folds)):
        ...
        scores = []
        for w in weights:
            scores.append(roc_auc_score(y[sel], w * rl[sel] + (1 - w) * rx[sel]))
...
def main():
    ...
    for f, (ti, vi) in enumerate(outer.split(train, y), 0):
        ...
        yy = y[ti]
        yv = y[vi]
        etr, eva, ete = fold_te(tr, yy, va, test.reset_index(drop=True), a.inner_folds, 10.0, a.seed + 100 + f)
        xl_tr = feature_frame(tr, etr, False)
        xl_va = feature_frame(va, eva, False)
        xl_te = feature_frame(test, ete, False)

        ml = lgb.LGBMClassifier(
            objective="binary", metric="auc", n_estimators=a.lgb_estimators,
            ...
            max_bin=255, random_state=a.seed + f, n_jobs=-1, verbosity=-1,
        )
        ml.fit(xl_tr, yy, eval_set=[(xl_va, yv)], eval_metric="auc",
        ...
        mx = xgb.XGBClassifier(
            ...
            n_estimators=a.xgb_estimators, learning_rate=0.035,
            max_depth=8, min_child_weight=20.0, subsample=0.9,
            colsample_bytree=0.9, reg_alpha=0.1, reg_lambda=2.0,
            max_bin=256, tree_method="hist", random_state=a.seed + f,
    ...
    honest, held_weights_lgb, mean_w_lgb = best_rotating_blend(y, fold_id, oof_lgb, oof_xgb)
    ...
    if selected == "lgb": final = test_lgb
    elif selected == "xgb": final = test_xgb
    else: final = test_blend

    for name, pred in (("lgb", test_lgb), ("xgb", test_xgb), ("blend", test_blend), ("next", final)):
        sub = sample[[ID]].copy()
        sub[TARGET] = pred
    ...
    pd.DataFrame({ID: train[ID], TARGET: y, "fold": fold_id,
                  "lgb": oof_lgb, "xgb": oof_xgb, "honest_blend": honest}).to_csv(out / "oof.csv", index=False)
    decision = {
        "version": "live-frontier-v1",
        "rows": {"train": len(train), "test": len(test)},
        "folds": a.folds, "inner_folds": a.inner_folds,
        "auc": candidates,
